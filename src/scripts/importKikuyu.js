...
export const importKikuyuContent = async () => {
  try {
    console.log('🚀 Starting Kikuyu content import...');
    
    const language = 'kikuyu';
    ...
    console.error('❌ Kikuyu import failed:', error);
    return {
      success: false,
      error: error.message,
...
// Quick test function
export const testKikuyuContent = async () => {
  try {
    console.log('🧪 Testing Kikuyu content...');
    
    const categories = ['basic_words', 'greetings', 'numbers', 'family', 'names'];
    
    for (const category of categories) {
      const collectionName = getCollectionName('kikuyu', category);
    ...
    console.log('✅ Kikuyu content test completed!');
